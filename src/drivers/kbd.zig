...
pub const Keyboard = struct {
    ...
    fn getKeyevent(self: *Keyboard, scancode: u8) ?KeyEvent {
        ...
        switch (enumcode) {
            ...
            .K_PGUP                 => {if (!release) return .{ .ctl = true, .val = @intFromEnum(CtrlType.PGUP) }; },
            .K_PGDN                 => {if (!release) return .{ .ctl = true, .val = @intFromEnum(CtrlType.PGDN) }; },
            .K_INS                  => {if (!release) return .{ .ctl = true, .val = @intFromEnum(CtrlType.INSERT) }; },
            .K_DEL                  => {if (!release) return .{ .ctl = true, .val = @intFromEnum(CtrlType.DELETE) }; },
            .K_F1,.K_F2,.K_F3,
            .K_F4,.K_F5,.K_F6,
