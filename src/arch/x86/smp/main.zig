...
pub fn init() void {
    ...
    const apic_page: u32 = krn.mm.virt_memory_manager.findFreeSpace(
        1,
        0xC0000000,
        0xFFFFF000,
        false
    );
    if (apic_page == 0xFFFFFFFF)
        return;
    const apic_base: u32 = madt.local_address;
    const phys_apic_address = arch.pageAlign(apic_base, true);
    ...
    while (madt.getNextEntry(madt_entry)) |entry| {
        if (entry.type == 0) {
            ...
            // krn.logger.INFO("Proc: {any}", .{proc_apic});
            // Skip CPUs that are neither enabled nor online-capable
            // (flags bit 0 = enabled, bit 1 = online-capable).
            if ((proc_apic.flags & 0b11) == 0) {
                // krn.logger.INFO("CPU {d} not usable, skipping\n", .{proc_apic.apic_id});
